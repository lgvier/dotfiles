...
hs.hotkey.bind({"ctrl","alt","cmd"}, "[", function()
	sizeup.send_window_left_70()
end)
-- Send Window Right 30%
hs.hotkey.bind({"ctrl","alt","cmd"}, "]", function()
	sizeup.send_window_right_30()
end)
...
sizeup.split_screen_partitions_70 = {
	x = 0.7, -- %
	y = 0.7
}
...
function sizeup.send_window_left_70()
	local s = sizeup.screen()
	local ssp = sizeup.split_screen_partitions_70
	local g = sizeup.gutter()
	sizeup.set_frame("Left 70%", {
		x = s.x,
		y = s.y,
		w = (s.w * ssp.x) - sizeup.gutter().x,
		h = s.h
...
function sizeup.send_window_right_30()
	local s = sizeup.screen()
	local ssp = sizeup.split_screen_partitions_70
	local g = sizeup.gutter()
	sizeup.set_frame("Right 30%", {
		x = s.x + (s.w * ssp.x) + g.x,
		y = s.y,
		w = (s.w * (1 - ssp.x)) - g.x,
		h = s.h
